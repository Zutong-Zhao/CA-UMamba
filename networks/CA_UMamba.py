...
from networks.VSSBlock import VSSBlock
...
class Encoder(nn.Module):
    def __init__(self, image_size, in_dim, key_dim, value_dim, layers, head_count=1, token_mlp="mix_skip"):
        ...
        self.block1 = nn.ModuleList(
            ...
            [VSSBlock(hidden_dim=in_dim[0],drop_path=0,norm_layer=norm_layer,attn_drop_rate=0,d_state=16) for _ in range(layers[0])]
        )
        self.norm1 = nn.LayerNorm(in_dim[0])

        self.block2 = nn.ModuleList(
            #[DualTransformerBlock(in_dim[1], key_dim[1], value_dim[1], head_count, token_mlp) for _ in range(layers[1])]
            [VSSBlock(hidden_dim=in_dim[1],drop_path=0,norm_layer=norm_layer,attn_drop_rate=0,d_state=16) for _ in range(layers[1])]
        )
        ...
        self.block3 = nn.ModuleList(
            ...
            [VSSBlock(hidden_dim=in_dim[2],drop_path=0,norm_layer=norm_layer,attn_drop_rate=0,d_state=16) for _ in range(layers[2])]
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_cnn = x_ch.permute(0, 2, 3, 1)#torch.Size([20, 56, 56, 128])
        ...
        cnn2 = self.p2(cnn)  # torch.Size([10, 512, 28, 28])
        x2_ch = self.p2_ch(cnn2)  # torch.Size([10, 320, 28, 28])
        x2_cnn = x2_ch.permute(0, 2, 3, 1)#torch.Size([10, 28, 28, 320])
        ...
        # stage 3
        cnn3 = self.p3(cnn2)  # torch.Size([10, 1024, 14, 14])
        x3_ch = self.p3_ch(cnn3)  # torch.Size([10, 512, 14, 14])
        x3_cnn = x3_ch.permute(0, 2, 3, 1)#torch.Size([10, 14, 14, 512])
...
class MyDecoderLayer(nn.Module):
    def __init__(
        ...
        self.layer_former_1 = VSSBlock(hidden_dim=out_dim, drop_path=0, norm_layer=norm_layer, attn_drop_rate=0, d_state=16)
        self.layer_former_2 = VSSBlock(hidden_dim=out_dim, drop_path=0, norm_layer=norm_layer, attn_drop_rate=0, d_state=16)
...
class myFormer(nn.Module):
    def __init__(self, num_classes=9, head_count=1, token_mlp_mode="mix_skip"):
        ...
        # Encoder
        dims, key_dim, value_dim, layers = [[32, 64, 128], [32, 64, 128], [32, 64, 128], [2, 2, 2]]
